...
def get_instruction(opcode: str, r_A: int, r_B: int, r_C: int) -> bitarray.bitarray:
    ...
    instr_bus[0:4] = bitarray.util.int2ba(
        INSTRUCTIONS[opcode], endian="little", length=INSTR_BITS
    )

    # r_A set by next four bits
    instr_bus[4:8] = bitarray.util.int2ba(r_A, endian="little", length=REG_BITS)

    # And then r_B
    instr_bus[8:12] = bitarray.util.int2ba(r_B, endian="little", length=REG_BITS)

    # r_C set by the final bits of the instruction
    instr_bus[12:16] = bitarray.util.int2ba(r_C, endian="little", length=REG_BITS)
